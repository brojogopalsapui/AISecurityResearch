...
def build_watch_article(data):
    ...
    meta_line = re.sub(r'\s*[•|:-]?\s*(Academic(?:\s+Signals?|\s+stream)?|Industry(?:\s*&\s*Innovation|\s+stream)?|Ecosystem(?:\s+stream)?|Companies\s*&\s*Releases)\s*$', '', meta_line, flags=re.IGNORECASE).strip(' •|-:')
    if stream_label.lower() not in meta_line.lower():
        meta_line = f'{meta_line} • {stream_label}' if meta_line else stream_label
    ...
    stream_chip = CATEGORY_LABELS.get(category, category.title())
    article_html = f'''
<article class="watch-note accordion" data-category="{escape(category, quote=True)}" id="{escape(post_id, quote=True)}">
  <button aria-expanded="false" class="accordion-trigger" type="button">
    <span class="stream-chip stream-chip--{escape(category, quote=True)}">{escape(stream_chip)}</span>
